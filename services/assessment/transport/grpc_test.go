...
func init() {
	...
	pb.RegisterassessmentServiceServer(s, server)
	go func() {
...
func setupClient(t *testing.T) (*grpc.ClientConn, pb.assessmentServiceClient) {
	conn, err := grpc.DialContext(ctx, "", grpc.WithContextDialer(bufDialer), grpc.WithInsecure())
	require.NoError(t, err)
	return conn, pb.NewassessmentServiceClient(conn)
}
